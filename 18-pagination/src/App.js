...
function App() {
  ...
  useEffect(() => {
    if (loading)
      return;
    else                        //gdy dane pobrano
      setFollowers(data[page]); //wyświetl partię profili o indeksie 'page'
  }, [loading, page])           //ale w chwili gdy przeładowano paginację
  ...
  const noah = () => {
    setPage((oldPage) => {
      console.log('Stary index: ', oldPage);
      return oldPage;
    })
